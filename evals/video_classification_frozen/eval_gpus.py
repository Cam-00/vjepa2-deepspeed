...
def main(args_eval, resume_preempt=False):
    ...
    ck_freq = args_eval.get("checkpoint_frequency")
    monitor_step = args_eval.get("monitor_step")
    ...
    batch_size = args_opt.get("batch_size")  # 单张GPU的批次大小
    ...
    # 获取节点内的本地排名，通常由启动工具提供
    local_rank = int(os.environ.get("LOCAL_RANK", rank % torch.cuda.device_count()))

    if not torch.cuda.is_available():
        device = torch.device("cpu")
    else:
        # 优先尝试逻辑上的第 0 张卡，因为如果环境被隔离了，每个进程看到的都是 cuda:0
        try:
            device = torch.device(f"cuda:{local_rank}")
            torch.cuda.set_device(device)
        except RuntimeError:
            # 如果报错，说明环境已经做了隔离，直接使用第一张可见卡即可
            device = torch.device("cuda:0")
            torch.cuda.set_device(device)

    logger.info(f"Initialized (rank/world-size) {rank}/{world_size} on {device}")
    ...
    eval_ipe = len(val_loader)
    logger.info(f"Train dataloader created... iterations per epoch: {ipe}")
    logger.info(f"Eval dataloader created... iterations per epoch: {eval_ipe}")
    ...
    if resume_checkpoint and os.path.exists(latest_path):
        classifiers, optimizer, scaler, start_epoch, start_itr = load_checkpoint(
            device=device,
            r_path=latest_path,
            classifiers=classifiers,
            opt=optimizer,
        ...
        for _ in range(start_epoch * ipe + start_itr):
            [s.step() for s in scheduler]
            [wds.step() for wds in wd_scheduler]

    # def save_checkpoint(epoch, itr):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if val_only:
            ...
            train_acc, train_losses = run_one_epoch(
                ...
                loader_itr=train_loader_itr,
            )

        torch.cuda.empty_cache()
        # print("cuda empty cache finished")

        start_itr = 0

        if not val_only:
            save_every_file = f"e{epoch + 1}.pt"
            epoch_path = os.path.join(folder, save_every_file)
            save_checkpoint(epoch, ipe, batch_size, world_size, rank, epoch_path, classifiers, scaler, optimizer)

        eval_loader_itr = iter(val_loader)
        val_acc, _ = run_one_epoch(
            ...
            latest_path=latest_path,
            monitor_step=monitor_step,
            loader_itr=eval_loader_itr,
        )

        logger.info("[%5d] train_loss: [%.3f %.3f] train_acc: %.3f%% eval_acc: %.3f%%"
                    % (epoch + 1, train_losses[0], train_losses[1], train_acc, val_acc))
        if rank == 0:
            csv_logger.log(epoch + 1, train_losses[0], train_losses[1], train_acc, val_acc)
...
def save_checkpoint(epoch, itr, batch_size, world_size, rank, latest_path, classifiers, scaler, optimizer):
    all_classifier_dicts = [c.state_dict() for c in classifiers]
    all_opt_dicts = [o.state_dict() for o in optimizer]
...
def run_one_epoch(
    ...
    ipe = len(data_loader)
    print("run_one_epoch: total ipe is ", ipe)

    # end_itr = 11 if training else 12
    print("run_one_epoch: start_itr: ", start_itr)
    # print("run_one_epoch: end_itr: ", ipe)
    for itr in range(start_itr, ipe):   # (start_itr, ipe)
        iter_retries = 0
        iter_successful = False
        while not iter_successful:
            try:
                # 获取下一批次数据, data 是数据, 不是迭代器
                # data: 一个元组 (buffer, label, clip_indices)
                data = next(loader_itr)  # loader 是一个迭代器, 指向 data_loader
                iter_successful = True
                # print("run_one_epoch: load data successfully")
                ...
                if iter_retries < NUM_RETRIES:
                    ...
                    raise e
        # 如果是eval, 只验证500步
        if not training and itr >= 500:
            break

        # if training:
        #     [s.step() for s in scheduler]
        #     [wds.step() for wds in wd_scheduler]
        active_dtype = torch.bfloat16 if use_bfloat16 else torch.float16
        with torch.amp.autocast(device_type='cuda', dtype=active_dtype, enabled=use_bfloat16):
        ...
        # print("run_one_epoch: loss is got")
        if training:
            if not use_bfloat16:
                [[s.scale(lij).backward() for lij in li] for s, li in zip(scaler, losses)]  # 使用torch.float16时才用
        ...
        # print("run_one_epoch: top1_meters is updated")
        ...
        if itr % 10 == 0:
            logger.info(
                "[%5d] [acc_avg: %.3f%% %.3f%%] [%.3f%% %.3f%% %.3f%%] [losses: %.3f %.3f] [mem: %.2e] [%.3f%% %.3f%%]"
                % (
                    itr,
                    _agg_top1[0],
                    _agg_top1[1],
        ...
        # 保存checkpoint
        if training:
            if (itr + 1) % ck_freq == 0 or (itr + 1) == ipe:
                save_checkpoint(epoch, itr + 1, batch_size_per_gpu, world_size, rank,
                                latest_path, classifiers, scaler, optimizer)
...
def load_checkpoint(device, r_path, classifiers, opt, scaler, val_only=False):
    ...
    if val_only:
        ...
        return classifiers, opt, scaler, 0, 0

    epoch = checkpoint["epoch"]
    itr = checkpoint["itr"]
    logger.info(f"loaded pretrained classifier from itr/epoch {itr}/{epoch + 1} with msg: {msg}")
    ...
    return classifiers, opt, scaler, epoch, itr
...
def make_dataloader(
    ...
    # 依据training，选择不同的数据变换处理：
    #   train: 采取tensor_normalize/随机大小尺寸剪裁/RandomErasing，增强模型泛化能力
    #   eval: 不做任何形状变换
...
def init_opt(classifiers, iterations_per_epoch, accumulation_steps, opt_kwargs, num_epochs, use_bfloat16=False):
    ...
    for c, kwargs in zip(classifiers, opt_kwargs):
        ...
        scalers += [torch.amp.GradScaler("cuda") if not use_bfloat16 else None]
...
class WarmupCosineLRSchedule(object):
    ...
    def step(self):
        ...
        for group in self.optimizer.param_groups:
            ...
            if prefix == 'no_decay':
                prefix = 'base'
            # ref_lr = group.get(prefix + '_ref_lr')  # 原代码
            # 修改为（增加对 None 的处理）：
            actual_prefix = prefix if prefix is not None else 'mc'
            ref_lr = group.get(actual_prefix + '_ref_lr')
            final_lr = group.get(actual_prefix + '_final_lr')
            start_lr = group.get(actual_prefix + '_start_lr')
    ...
    def step(self):
        ...
        for group in self.optimizer.param_groups:
            ...
            if prefix == 'no_decay':
                ...
                actual_prefix = prefix if prefix is not None else 'mc'
                ref_wd = group.get(actual_prefix + '_ref_wd')
                final_wd = group.get(actual_prefix + '_final_wd')
